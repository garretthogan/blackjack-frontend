...
export default function BettingScreen({ min = 5, max = 500, denoms = [1, 5, 25, 100, 500], onDeal }) {
    ...
    const onKeyDown = (e) => {
        ...
        if (idx !== -1 && idx < denoms.length) { setSelectedDenom(denoms[idx]); return; }
    ...
    const onCircleKeyDown = (e) => {
        if (e.key === " " || e.key === "Enter") { e.preventDefault(); addChip(selectedDenom); }
    };

    return (
        <div ref={rootRef} tabIndex={-1} onKeyDown={onKeyDown} className="min-h-screen outline-none">
            {/* Header */}
            <header className="border-b border-zinc-200 backdrop-blur">
                <div className="mx-auto flex max-w-5xl items-center justify-between p-4">
                    ...
                    <div className="text-sm">Table Limits: ${min}–${max}</div>
                </div>
            </header>
                ...
                <section className="rounded-2xl border border-zinc-200 p-6 shadow-sm">
                    ...
                    <div className="flex flex-col items-center gap-4 py-8">
                        <div
                            ...
                            onContextMenu={(e) => { e.preventDefault(); removeChip(selectedDenom); }}
                        ...
                        >
                            ...
                            <div className="absolute -bottom-3 left-1/2 flex -translate-x-1/2 gap-2">
                                <button
                                    onClick={(e) => { e.stopPropagation(); removeChip(selectedDenom); }}
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); addChip(selectedDenom); }}
                                    className="h-8 w-8 rounded-full border border-zinc-300 text-sm font-semibold shadow-sm hover:bg-zinc-50"
                        ...
                        <div className="text-xs">
                            {denoms.map((d) => (
                                <span key={d} className="mr-3">${d}: <span className="font-medium">{chipCounts[d] || 0}</span></span>
                            ))}
                        </div>
                    </div>
                        ...
                        {denoms.map((d, i) => (
                            <button
                                ...
                                className={
                                    ...
                                    (selectedDenom === d ? "border-zinc-900 ring-zinc-400" : "border-zinc-300")
                                }
                    ...
                    <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
                        ...
                        <button
                            ...
                            className={
                                ...
                                (total === 0 || !withinLimits ? "cursor-not-allowed border border-zinc-200 bg-zinc-100 text-zinc-400" : "bg-zinc-900 text-white hover:opacity-95 focus-visible:ring-zinc-400")
                            }
                            title="Enter"
                            type="button"
                        >
                    ...
                    <p className="mt-6 text-center text-xs">
                        <kbd>1–{Math.min(9, denoms.length)}</kbd> select chip · <kbd>←/→</kbd> cycle · <kbd>+</kbd>/<kbd>-</kbd> adjust · <kbd>Enter</kbd> deal · <kbd>Backspace</kbd> clear
                    </p>
                    <p className="mt-2 text-center text-xs">Table limits: ${min}–${max}</p>
                </section>
            </main>
        </div>
    );
